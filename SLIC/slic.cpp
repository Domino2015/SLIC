...
cv::Point  Slic::find_local_minimum(cv::Mat &image, cv::Point  center) {
    ...
    for (int i = center.x - 1; i < center.x + 2; ++i) {
        for (int j = center.y - 1; j < center.y + 2; ++j) {
            // 检查数组边界
            if (i < 0 || i >= image.cols || j < 0 || j >= image.rows) continue;
            ...
            // 转换颜色值为灰度值（如果需要的话）
            double i1 = c1[0]; // 或者其他转灰度的算法
            double i2 = c2[0];
            double i3 = c3[0];
            // double i1 = c1[0] * 0.11 + c1[1] * 0.59 + c1[2] * 0.3;
            // double i2 = c2[0] * 0.11 + c2[1] * 0.59 + c2[2] * 0.3;
            // double i3 = c3[0] * 0.11 + c3[1] * 0.59 + c3[2] * 0.3;
    ...
    for (int i = 0; i < NR_ITERATIONS; i++) {
            ...
            for (int k = centers[j][3] - step; k < centers[j][3] + step; k++) {
                for (int l = centers[j][4] - step; l < centers[j][4] + step; l++) {
                    ...
                    if (k >= 0 && k < image.cols && l >= 0 && l < image.rows) {
                        cv::Vec3b vecColour = image.at<cv::Vec3b>(l, k);
                        cv::Scalar colour(vecColour[0], vecColour[1], vecColour[2]);
        ...
        for (int j = 0; j < image.cols; j++) {
            for (int k = 0; k < image.rows; k++) {
                ...
                if (c_id != -1) {
                    cv::Vec3b vecColour = image.at<cv::Vec3b>(k, j);
                    cv::Scalar colour(vecColour[0], vecColour[1], vecColour[2]);
...
void Slic::display_contours(cv::Mat &image, cv::Scalar colour) {
    ...
    for (int i = 0; i < (int)contours.size(); i++) {
        cv::Vec3b& pixel = image.at<cv::Vec3b>(contours[i].y, contours[i].x);
        pixel[0] = static_cast<uchar>(colour[0]);
        pixel[1] = static_cast<uchar>(colour[1]);
        pixel[2] = static_cast<uchar>(colour[2]);
    }
}
...
void Slic::colour_with_cluster_means(cv::Mat& image) {
    std::vector<cv::Scalar> colours(centers.size(), cv::Scalar(0, 0, 0));
    std::vector<int> center_counts(centers.size(), 0);  // 假设你已经有这个数组，用于计算平均值
    ...
    for (int i = 0; i < image.cols; i++) {
        for (int j = 0; j < image.rows; j++) {
            ...
            image.at<cv::Vec3b>(j, i) = cv::Vec3b(static_cast<uchar>(ncolour[0]),
                static_cast<uchar>(ncolour[1]),
                static_cast<uchar>(ncolour[2]));
        }
    }
}
